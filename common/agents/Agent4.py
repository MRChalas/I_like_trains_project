...
class Node:
    ...
    def __eq__(self, node):
        #isinstance to avoid error if type is not the same
...
class Agent(BaseAgent):
    ...
    def distance_to_point(self, a:tuple, b:tuple):
        """
        Calculates shortest distance between the train and a given point

        IN:  coordinates (x,y) of points a and b
    ...
    def closest_passenger(self):
        ...
        for pos in passenger_positions:
            ...
            if self.distance_to_point(self.train_position, pos) < shortest_distance:
                shortest_distance = self.distance_to_point(self.train_position, pos)
                closest_passenger = pos

        return closest_passenger

    def closest_delivery_point(self):
        """
    ...
    def get_neighbors(self, current_point, grid_with_obstacles):
        """
        Determines neighboring positions for a given point
        
        IN; (x,y) coordinates (tuple)
        OUT: list of neighboring positions (list of tuples)"""
        self.positions()
        neighbor_pos = []
        for move in self.moves:
            ...
            if (0 <= x_neighbor_pos < self.game_width) and (0<= y_neighbor_pos < self.game_height):
                if grid_with_obstacles[y_neighbor_pos // self.cell_size][x_neighbor_pos // self.cell_size] != self.OCCUPIED:
                    neighbor_pos.append((x_neighbor_pos, y_neighbor_pos))
    ...
    def grid_with_obstacles(self):
        ...
        self.positions()
        grid = [[self.AVAILABLE for x in range(self.game_width // self.cell_size)]
                for y in range(self.game_height // self.cell_size)]

        for train in self.all_trains:
            train_pos = [tuple(self.all_trains[train]["position"])]
            wagons = [tuple(wagon_pos) for wagon_pos in self.all_trains[train]["wagons"]]
            for pos in [train_pos] + wagons:
                grid[pos[1] // self.cell_size][pos[0] // self.cell_size] = self.OCCUPIED

            if train != self.nickname:
                for move in self.moves:
                    neighbor = self.new_position(train_pos, move)
                    grid[neighbor[1] // self.cell_size][neighbor[0] // self.cell_size] = self.AVOID
        return grid
    ...
    def path_to_point(self, goal):
        ...
        points_to_evaluate, evaluated_points = [start_point], set()
        ...
        while points_to_evaluate:
            ...
            for neighbor_pos in self.get_neighbors(current_point.position, grid):
                ...
                neighbor_point = Node(neighbor_pos, current_point.real_cost + cost_of_move, self.distance_to_point(neighbor_pos, goal), current_point)
    ...
    def get_direction(self, path):
        for move in self.moves:
            if self.new_position(self.train_position, move, 1) == path:
                return Move(move)
    ...
    def delivery_donut(self):
        ...
        launch_circling = tuple(self.train_position) in delivery_spots
