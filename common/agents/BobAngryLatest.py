...
class Agent(BaseAgent):
    ...
    def positions(self):
        ...
        #define ultimate_strategy as initially disabled
            ...
            if train == self.nickname:
               continue
                ...
                for move in moves:
                   ...
                   precaution_position2=self.new_position(head_position,move,2)
                   #avoids duplicates
                   if precaution_position not in head_positions:
                        head_positions.append(precaution_position)
                    
                   if precaution_position2 not in head_positions:
                         head_positions.append(precaution_position2)
    ...
    def path_to_point(self, point: tuple):
        ...
        for move in moves:
            ...
            if tuple(new_pos) in available_positions:
                    #differentiates possible positions from safe positions
    ...
    def get_move(self):
        ...
        the delivery zone automatically when our agent leads by at least 9 points. This ensures victory as
        the ennemy cannot reach the delivery zone anymore."""
        perimeter=2*self.delivery_zone['width']/self.cell_size  +   2*self.delivery_zone['height']/self.cell_size
        perimeter=perimeter+4 #+4 for the corners
        maxscore=0
        ...
        if len(self.all_trains[self.nickname]['wagons'])>=3:
            DELIVER = 1

        if self.best_scores and self.nickname in self.best_scores:
            if self.best_scores[self.nickname]>maxscore+9 and  len(self.all_trains[self.nickname]['wagons'])<perimeter and delivery_position_not_on_edge :
                DELIVER=0
        ...
        #the code belows enables the "ultimate strategy" when the lead is more than 9 points.
        #we drop one wagon if possible to reach the perfect score, otherwise we do without it
        if self.best_scores and self.nickname in self.best_scores and delivery_position_not_on_edge and len(self.all_trains)!=4 and len(self.all_trains)!=3:
            #the ultimate strategy is very strong in 1v1, but disastrous in 1v1v1v1
            if self.best_scores[self.nickname]>maxscore+9:
                if len(self.all_trains[self.nickname]['wagons'])==perimeter-1 and not self.ultimate_strategy:
