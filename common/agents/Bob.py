...
class Agent(BaseAgent):
    ...
    def near_to_head(self):
        list_position_heads=[]
        for train in self.all_trains:
            if train==self.nickname:
                continue
            else:
                list_position_heads.append(self.all_trains[train]["position"])
                moves = [Move.LEFT.value, Move.DOWN.value, Move.UP.value,  Move.RIGHT.value]
                for move in moves:
                    positiontoadd=self.new_position(move,1)
                    list_position_heads.append(positiontoadd)
